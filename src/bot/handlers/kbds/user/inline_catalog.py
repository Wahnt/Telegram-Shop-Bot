...
"""Модуль для инлайн кнопок каталога"""
...
def get_user_main_btns(*, level: int, sizes: tuple[int] = (2, 2, 2, 1)):
    """Главное меню"""
...
def get_user_catalog_btns(
        *, level: int, categories: list, sizes: tuple[int] = (2, 2)):
    """Каталог"""
    keyboard = InlineKeyboardBuilder()

    keyboard.add(InlineKeyboardButton(text='⬅️ Назад',
...
def get_back_btns(
    *,
    level: int,
        sizes: tuple[int] = (1,)):
    """Метод заглушка для выхода(level-1)"""
    keyboard = InlineKeyboardBuilder()
